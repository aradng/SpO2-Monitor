...
//ok
void MAX30100_SetHiRes(bool enable){
	uint8_t spo2Reg = MAX30100_ReadReg(MAX30100_SPO2_CONFIG);
	spo2Reg = ((enable << MAX30100_SPO2_HI_RES_ENB) & 0x40) | (spo2Reg & ~0x40);
...
uint8_t MAX30100_ReadFIFO(void){
	...
	//fifo_wr_ptr = MAX30100_ReadReg(MAX30100_FIFO_WR_PTR);
	//fifo_rd_ptr = MAX30100_ReadReg(MAX30100_FIFO_RD_PTR);
	//uint8_t		ovf= MAX30100_ReadReg(MAX30100_OVF_COUNTER);
	//uint8_t		inter= MAX30100_ReadReg(MAX30100_INTERRUPT);
	//printf("fifo pointer : %u\t%u\t%u\t%u\r\n" , fifo_wr_ptr , fifo_rd_ptr , ovf , inter);
	return num_sample/4;
}
...
void MAX30100_Reset(void){
	...
	MAX30100_WriteReg(MAX30100_MODE_CONFIG, (0x01 << MAX30100_RESET));
}
