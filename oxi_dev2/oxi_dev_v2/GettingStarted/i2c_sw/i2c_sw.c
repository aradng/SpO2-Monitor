...
/*void GPIO_SetBits(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin)             //uncomment for hal
{
...
uint8_t SW_I2C_UTIL_WRITE_Multi(uint8_t sel, uint8_t IICID, uint8_t regaddr, uint8_t rcnt, uint8_t (*pdata))
{
	return SW_I2C_WritenControl_8Bit(sel, IICID<<1, regaddr, rcnt, pdata);
}
